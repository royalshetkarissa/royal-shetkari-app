...
class ShopRepository {
  async create(data) {
    const { name, profile_photo, address, contact_mobile, whatsapp_number, categories, images, latitude, longitude, ownerId, owner_name, services, pincode, city } = data;
    const result = await pool.query(
      ...
      [name, profile_photo, address, contact_mobile, whatsapp_number, JSON.stringify(categories), JSON.stringify(images), latitude, longitude, ownerId, owner_name, services, pincode, city]
    );
  ...
  async findAdminShops() {
    const result = await pool.query(`SELECT * FROM shops WHERE status != 'deleted' ORDER BY created_at DESC`);
    return result.rows;
  }

  async updateStatus(id, status) {
    const result = await pool.query(`UPDATE shops SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [status, id]);
    return result.rows[0];
  }
  ...
  async logClick(shopId, userId, clickType) {
    await pool.query(`INSERT INTO shop_clicks (shop_id, user_id, click_type) VALUES ($1, $2, $3)`, [shopId, userId, clickType]);
  }
