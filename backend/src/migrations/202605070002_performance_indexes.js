...
exports.up = async (client) => {
  ...
  await client.query('CREATE INDEX IF NOT EXISTS idx_posts_cat_status_date ON posts(category, status, created_at DESC)');
  ...
  await client.query('CREATE INDEX IF NOT EXISTS idx_otps_mobile_expiry ON otps(mobile, expires_at)');
  
  // Activity Logs: GIN index for fast JSONB search
  await client.query('CREATE INDEX IF NOT EXISTS idx_activity_logs_details ON activity_logs USING GIN (details)');
};
