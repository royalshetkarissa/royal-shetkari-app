...
exports.up = async (client) => {
  ...
  await client.query(`CREATE INDEX IF NOT EXISTS idx_journeys_user_status ON user_crop_journeys(user_id, status);`);
  
  // Index for retrieving tasks for a journey
  await client.query(`CREATE INDEX IF NOT EXISTS idx_tasks_journey_id ON user_crop_tasks(user_crop_id);`);
  
  // Index for activity logging queries
  await client.query(`CREATE INDEX IF NOT EXISTS idx_activity_user_type ON activity_logs(user_id, action_type);`);
