...
exports.up = async (client) => {
  ...
  for (const [name, [min, max]] of Object.entries(durations)) {
    await client.query(
      `UPDATE crops SET harvest_days_min = $1, harvest_days_max = $2 WHERE name = $3`,
      [min, max, name]
    );
  }
  ...
  for (const cropName of cropsToSeed) {
    const cropId = (await client.query(`SELECT id FROM crops WHERE name = $1`, [cropName])).rows[0]
      .id;
    ...
    if (cropName === 'Cauliflower') {
      templateData = [
        ...
        [
          ...
          'Harvesting Stage',
          'काढणी अवस्था',
          'Check for curd firmness',
          'SOP (0:0:50) for quality',
        ],
      ];
      ...
      templateData = [
        ...
        [
          ...
          'मान फुगवण',
          'Dashparni Ark spray',
          'Urea + Micronutrient Mixture',
        ],
        [
          60,
          'Bulb Initiation',
          'कांदा धरण्याची अवस्था',
          'Potash rich organic manure',
          '0:52:34 (5kg)',
        ],
